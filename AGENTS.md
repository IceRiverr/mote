...
任何存在多种合理实现路径时，不要默默选择。按决策成本分级响应：

### 快速决策（编码中）
- **触发**：用户问"这地方是否应该…""这里用 A 还是 B…""是否可以…"，或你在编码中发现两条以上合理路径。
- **流程**：STOP → 一句话命名歧义 → 2-3 个选项 + 利弊 → 问用户 → **不强制文档**。
- **不适用**：显而易见、只有一条合理路径、纯本地实现细节。

### 重型决策（架构 / 数据模型 / 破坏性变更）
- **触发**：两套可能的架构、与现有模式冲突、序列化格式变更、推翻之前设计决策、关键上下文缺失。
- **流程**：STOP → 正式命名歧义 → 评估影响面 → 2-3 个选项 + 利弊 + 风险等级 → 问用户 → 可能需要文档化。
- **绝不猜测**：不要在架构或数据模型决策上替用户做主。
