...
- `store/scene.ts` 新增 `lastSelectedEntityId` signal，追踪最后点击的实体
- `drawEntity()` 增加 `isLastSelected` 参数，实现三级视觉层级：
  - 活动项（最后点击）：`#ffbb5c` 2px 轮廓 + 3px 轴心点
  - 非活动选择项：`#c4802a` 1px 轮廓 + 2px 轴心点
  - 未选中：无轮廓
→ 验证：点击实体后选择框大小精确匹配精灵图片；框选多实体后，最后点击的实体轮廓最亮，其他较暗

### ✅ 功能 8：选择工具拖拽实体 = 移动（Blender 风格）
- `ViewportCanvas.tsx` 实体模式 `onPointerDown` 区分 `select` / `move` 工具：
  - `select`：点击实体 = 选中 + 准备移动；Ctrl+点击 = 切换选择；点击空白 = 框选
  - `move`：无论点击哪里，只要有选中的实体就直接移动（不框选）
- `onPointerUp` 中移动结束提交 `MoveEntitiesCommand` 入 Undo 栈
- 吸附通过 `moveEntity()` 自动应用
→ 验证：选择工具下，在实体上按住拖拽可以直接移动；在空白处拖拽出现框选矩形；框选松开时矩形内的实体被选中；移动后按 Ctrl+Z 可撤销；move 工具下点击空白不会框选而是直接移动已选实体
