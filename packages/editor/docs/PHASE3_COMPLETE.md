...
| 测试文件 | 测试数 | 说明 |
|---------|--------|------|
| SetTileCommand.test.ts | 14 | Tile 命令 |
| BrushToolNew.test.ts | 16 | Tilemap 工具 |
| FloatingPanelDrag.test.tsx | 10 | 浮动面板拖拽 + pointerEvents 回归测试 |
...
**修复**：在面板容器上设置 `pointerEvents: 'none'`，在面板组件上设置 `pointerEvents: 'auto'`。

**自动化测试**：`FloatingPanelDrag.test.tsx` 包含 10 个测试用例。

#### 3. Tilemap 浮动面板不显示问题

**问题**：Tilemap 面板使用了 `window.innerWidth/Height` 计算位置，在 SSR/初始渲染时可能计算出屏幕外坐标，导致点击 T 按钮后面板不可见。

**修复**：
- 使用固定的初始位置 (100, 100)
- 添加 `typeof window !== 'undefined'` 检查确保 SSR 安全

**代码变更**：
```typescript
// 修复前
tilemap: { x: window.innerWidth / 2 - 300, y: window.innerHeight - 450 }

// 修复后
tilemap: { x: 100, y: 100 }  // 固定位置，确保可见
```
