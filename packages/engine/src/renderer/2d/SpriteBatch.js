...
export class TextureAtlas {
    ...
    bindGroup;
    regions = new Map();
    constructor(texture, bindGroup) {
        this.texture = texture;
        this.bindGroup = bindGroup;
    }
    static async load(gfx, imageUrl, jsonUrl) {
    ...
    getRegion(name) {
        ...
        if (!r)
            throw new Error(`Atlas region not found: ${name}`);
        return r;
    }
    get fullRegion() { return this.regions.get('__full__'); }
}
// ── SpriteBatch ───────────────────────────────────────────────────────────────
export class SpriteBatch {
    static MAX_QUADS = 10_000;
    static FLOATS_PER_VERTEX = 8; // pos(2) + uv(2) + color(4)
    static VERTICES_PER_QUAD = 4;
    static INDICES_PER_QUAD = 6;
    static VERTEX_STRIDE = 32; // 8 floats × 4 bytes
    static _atlasLayout = null;
    gfx;
    pipeline;
    vertexBuffer;
    indexBuffer;
    ...
    static getAtlasBindGroupLayout(_gfx) {
        if (!SpriteBatch._atlasLayout)
            throw new Error('SpriteBatch not yet constructed');
    ...
    drawQuad(x, y, w, h, rotation, region, atlas, color = { r: 1, g: 1, b: 1, a: 1 }) {
        if (this.quadCount >= SpriteBatch.MAX_QUADS)
            this._flush();
        if (atlas !== this.currentAtlas)
            this._breakBatch(atlas);
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function generateIndices(maxQuads) {
